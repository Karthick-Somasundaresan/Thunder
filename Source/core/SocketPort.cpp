...
#define WATCHDOG_ENABLED
    ...
    void SocketPort::BufferAlignment(SOCKET socket)
    {
        ...
        if (origSocketReceiveBufferSize != static_cast<uint32_t>(~0)) {
            // Also allow setting a 0 size buffer for kernel to allocate the required mininum.
            if (::setsockopt(socket, SOL_SOCKET, SO_RCVBUF, (const char*)&m_SocketReceiveBufferSize, sizeof(m_SocketReceiveBufferSize)) == SOCKET_ERROR) {
                TRACE_L1("Error could not set socket receive buffer size (%d).", m_SocketReceiveBufferSize);
            }
        }

        ::getsockopt(socket, SOL_SOCKET, SO_RCVBUF, (char*)&m_SocketReceiveBufferSize, &valueLength);
        ...
        if (origSocketReceiveBufferSize != m_SocketReceiveBufferSize) {
            TRACE_L2("Adjusted socket receive buffer size (%d->%d)", origSocketReceiveBufferSize, m_SocketReceiveBufferSize);
        }

        if ((m_ReceiveBufferSize == static_cast<uint16_t>(~0)) && (origSocketReceiveBufferSize == static_cast<uint32_t>(~0))) {
            m_ReceiveBufferSize = (m_SocketReceiveBufferSize > 0xFFFF? 0xFFFF : static_cast<uint16_t>(m_SocketReceiveBufferSize));
            TRACE_L1("Adjusted user receive buffer size (65535->%d)", m_SocketReceiveBufferSize);
        }

        const uint32_t origSocketSendBufferSize = m_SocketSendBufferSize;

        if (origSocketSendBufferSize != static_cast<uint32_t>(~0)) {
            if (::setsockopt(socket, SOL_SOCKET, SO_SNDBUF, (const char*)&m_SocketSendBufferSize, sizeof(m_SocketSendBufferSize)) == SOCKET_ERROR) {
                TRACE_L1("Error could not set socket send buffer size (%d).", m_SocketSendBufferSize);
            }
        }
        ...
        if (origSocketSendBufferSize != m_SocketSendBufferSize) {
            TRACE_L1("Adjusted socket send buffer size (%d->%d)", origSocketSendBufferSize, m_SocketSendBufferSize);
        }

        if ((m_SendBufferSize == static_cast<uint16_t>(~0)) && (origSocketSendBufferSize == static_cast<uint32_t>(~0))) {
            m_SendBufferSize = (m_SocketSendBufferSize > 0xFFFF? 0xFFFF : static_cast<uint16_t>(m_SocketSendBufferSize));
            TRACE_L2("Adjusted user send buffer size (65535->%d)", m_SocketSendBufferSize);
        }

        if ((m_ReceiveBufferSize != 0) || (m_SendBufferSize != 0)) {
            uint8_t* allocatedMemory = static_cast<uint8_t*>(::calloc(m_ReceiveBufferSize + m_SendBufferSize, 1));
